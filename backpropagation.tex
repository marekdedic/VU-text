...
A deep feedforward neural network (also known as a \VUname{multilayer perceptron}) can be seen as a special case of a computational graph. Each layer can be represented as
\[ \VUvec y = f \left( \VUvec{x} ; \VUvec{\theta} \right) = \sigma \left( \VUmat{W}^T \VUvec x + \VUvec{b} \right) \]
where \( \VUvec{x} \) is the input of the layer, \( \VUvec{y} \) its output and
\[ \VUvec{\theta} = \left( \VUmat{W}^T \middle| \VUvec{b} \right)^T \]
are the parameters of the layer (the weights \( \VUmat{W} \) and the bias \( \VUvec{b} \)).

If the \( i \)-th layer is denoted \( f^{(i)} \), then a neural network with input \( \VUvec{x} \) can be represented as
\[ J \circ f^{(n)} \circ f^{(n - 1)} \circ \dots \circ f^{(1)} \left( \VUvec{x} \right) \]
where for each layer \( f^{(i)} \) there are corresponding parameters \( \VUvec{\theta}^{(i)} \). \( J \) is the activation function of the network.
