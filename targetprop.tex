...
\begin{theorem}\todo{Theorem 1 from source}
\end{theorem}
...
\begin{remark}
	...
	\[ \widehat{\VUvec{h}}_{DTP}^{(i - 1)} = {f^{(i)}}^{-1} \left( \widehat{\VUvec{h}}^{(i)} \right) + \VUvec{h}^{(i - 1)} - {f^{(i)}}^{-1} \left( \VUvec{h}^{(i)} \right) = \]
	\[ = {f^{(i)}}^{-1} \left( \widehat{\VUvec{h}}^{(i)} \right) + \VUvec{h}^{(i - 1)} - \VUvec{h}^{(i - 1)} = {f^{(i)}}^{-1} \left( \widehat{\VUvec{h}}^{(i)} \right) = \widehat{\VUvec{h}}_{VTP}^{(i - 1)} \]
...
\begin{theorem}\todo{Theorem 2 from source}
\end{theorem}
